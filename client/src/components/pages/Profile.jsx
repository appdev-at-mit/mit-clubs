...
import { getSavedClubs, getUserMemberships, getUserData } from "../../api/clubs.js";
...
function Profile() {
  ...
  useEffect(() => {
    async function fetchUserData() {
      setLoading(true);
      try {
        // use the unified endpoint to get all user data in one request
        ...
        try {
          ...
          setSavedClubs(savedResponse.data || []);
        } catch (savedErr) {
  ...
  const renderClubList = (clubs, type) => {
    ...
    return (
      <div className="space-y-3">
        {clubs.map((club) => (
          <div key={club.club_id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div className="p-6">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-gray-600 text-sm mt-1">{userEmail || "No email"}</p>
                  </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden mt-6">
                ...
                <div className="p-6">
                  <div className="space-y-4">
                    {/* 
                    <button className="w-full py-2 px-4 bg-gray-100 hover:bg-gray-200 text-gray-800 text-sm rounded transition-colors">
                      Edit Profile
                    </button>
                    <button className="w-full py-2 px-4 bg-gray-100 hover:bg-gray-200 text-gray-800 text-sm rounded transition-colors">
                    ...
                    */}
                    <p className="text-sm text-gray-500 text-center">Settings coming soon</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                ...
                <div className="p-6">
                  {loading ? (
                    ...
                    <>
                      {activeTab === "saved" && renderClubList(savedClubs, "saved")}
                      {activeTab === "member" && renderClubList(memberClubs, "member")}
                    </>
                  )}
                </div>
              </div>
